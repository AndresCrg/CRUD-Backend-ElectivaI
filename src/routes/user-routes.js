...
router.post(
	...
	[
		...
		check('tipo_documento', 'No es un tipo de documento valido!').not().isEmpty().isIn(['CC', 'TI', 'CE', 'PA']),
		...
		check('fecha_fin_contrato', 'Debe ingresar una fecha válida!').not().isEmpty().isDate(),
		check('correo_electronico', 'El correo electrónico no es valido!').not().isEmpty().isEmail(),
...
router.patch(
	...
	[validateJWT, check('documento', 'El número de documento es obligatorio!').not().isEmpty(), check('fecha_baja', 'Debe ingresar una fecha válida!').isDate()],
	patchUSer
);
